...
<div class="main-page">
	...
	<div class="container">
		...
		<ul class="b2-cn-posts">
			...
			$getCats = mysql_query("SELECT * FROM newblog_cats WHERE cat_link = '$category'");
			if(mysql_num_rows($getCats) > 0){

				$cat = mysql_fetch_assoc($getCats);
				$cat_name = strtolower($cat['cat_name']);

				if($_SESSION['author']){ $check_cats = mysql_query("SELECT * FROM newblog_posts WHERE post_cats LIKE '%$cat_name%' AND post_deleted = 0"); }else{
				$check_cats = mysql_query("SELECT * FROM newblog_posts WHERE post_cats LIKE '%$cat_name%' AND post_deleted = 0 AND post_draft = 0"); }

				if(mysql_num_rows($check_cats) > 0){

				echo "<h1 class='result-returns'>Category: $cat_name <span>".mysql_num_rows($check_cats)." Results</span></h1>";

				while($post = mysql_fetch_assoc($check_cats)){ ?>
					...
					<?php }}else{
						...
						echo "No posts exist under that category.";
