...
function login_true($user){
	...
	echo "<meta http-equiv='refresh' content='0;URL=http://www.pavatimarine.com/blog/write/dashboard/'>";
...
function login_false(){
	...
	$db_user = mysql_query("SELECT * FROM authors WHERE author = '".$username."'");
	if(mysql_num_rows($db_user) < 1){ notify("error", "This username does not exist"); }else{
	$rt_user = mysql_fetch_assoc($db_user); $rt_pass = $rt_user['password']; $rt_last = $rt_user['last_login'];
	if($password === $rt_pass){ notify("success", "Logging you in, $username"); login($username); }else{ notify("error", "Wrong Password"); }}}
